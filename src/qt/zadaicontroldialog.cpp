// Copyright (c) 2017 The BITZONECOIN developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zadaicontroldialog.h"
#include "ui_zadaicontroldialog.h"
...
std::list<std::string> ZAdaiControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAdaiControlDialog::listMints;

ZAdaiControlDialog::ZAdaiControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAdaiControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAdaiControlDialog::~ZAdaiControlDialog()
{
    delete ui;
}

void ZAdaiControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAdaiControlDialog::updateList()
{
...
void ZAdaiControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAdaiControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAdai_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAdaiControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAdaiControlDialog::GetSelectedMints()
{
...
void ZAdaiControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
