                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zADAI ought to be enough for anybody." - Bill Gates, 2017
    ui->zADAIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzADAISyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzADAI->setEnabled(false);
        ui->pushButtonMintzADAI->setToolTip(tr("zADAI is currently disabled due to maintenance."));

        ui->pushButtonSpendzADAI->setEnabled(false);
        ui->pushButtonSpendzADAI->setToolTip(tr("zADAI is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zADAIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzADAI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADAI is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zADAI...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zADAI in ") + 
...
void PrivacyDialog::on_pushButtonSpendzADAI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADAI is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zADAI
        sendzADAI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zADAI
    sendzADAI();
}

void PrivacyDialog::on_pushButtonZAdaiControl_clicked()
{
    ZAdaiControlDialog* zAdaiControl = new ZAdaiControlDialog(this);
    zAdaiControl->setModel(walletModel);
    zAdaiControl->exec();
}

void PrivacyDialog::setZAdaiControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAdaiSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzADAI()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BitZoneCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zADAIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zADAI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ADAI </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zADAI</b>";
    ...
    // use mints from zAdai selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAdaiControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAdaiControlDialog::GetSelectedMints();
    }
    ...
    // Clear zadai selector in case it was used
    ZAdaiControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAdai Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Adai, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAdai Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zADAI </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zADAI "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zADAI "));
    ui->labelzADAIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzADAISyncStatus->setVisible(fShow);
}
