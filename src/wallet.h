...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ADAI at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zADAI send
enum ZerocoinSpendStatus {
    ZADAI_SPEND_OKAY = 0,                            // No error
    ZADAI_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZADAI_WALLET_LOCKED = 2,                         // Wallet was locked
    ZADAI_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZADAI_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZADAI_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZADAI_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZADAI_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZADAI_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZADAI_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZADAI_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZADAI_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZADAI_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZADAI_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZADAI_SPENT_USED_ZADAI = 14,                      // Coin has already been spend
    ZADAI_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZAdaiAutoBackups(bool fEnabled)
    {
